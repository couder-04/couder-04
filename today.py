...
def simple_request(func_name, query, variables):
    ...
    if request.status_code == 401:
        raise Exception(f"{func_name} failed: 401 Unauthorized (invalid or expired ACCESS_TOKEN). Response: {request.text}")
...
def loc_counter_one_repo(owner, repo_name, data, cache_comment, history, addition_total, deletion_total, my_commits):
    """Recursively call recursive_loc, only counting commits authored by me"""
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=None):
    """Uses GitHub's GraphQL v4 API to query all repositories and return the total lines of code"""
            ...
            repositories(first: 60, after: $cursor, ownerAffiliations: $owner_affiliation) {
                edges {
                    node {
                        ... on Repository {
                            nameWithOwner
                            defaultBranchRef { target { ... on Commit { history(author: {id: $author_id}) { totalCount } } } }
                        }
                    }
                }
                pageInfo { endCursor hasNextPage }
...
def force_close_file(data, cache_comment):
    """Forces the cache file to close, preserving whatever data was written to it, in case of a crash."""
    filename = 'cache/' + hashlib.sha256(USER_NAME.encode('utf-8')).hexdigest() + '.txt'
    with open(filename, 'w') as f:
        f.writelines(cache_comment)
        f.writelines(data)
...
def justify_format(root, element_id, new_text, length=0):
    """Updates the text of the element and adjusts the dots on the SVG for justification"""
...
if __name__ == '__main__':
    """
    Parth Tomar (couder-04)
    Based on the profile-stats generator originally by Andrew Grant (Andrew6rant), 2022-2025
    """
